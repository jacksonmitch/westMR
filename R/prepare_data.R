...
WMRData <- R6::R6Class(
  ...
  active = list(
    y     = function(v) { if (missing(v)) private$.y     else stop("y is read-only")     },
    X_het = function(v) { if (missing(v)) private$.X_het else stop("X_het is read-only") },
    X_com = function(v) { if (missing(v)) private$.X_com else stop("X_com is read-only") },
